(function () {
  ...
  const CONFIG = {
    ...
    chainSlug: 'robinhood',
    nativeSymbol: 'ETH',
    launchpadUrl: 'https://fun.noxa.fi/robinhood',
  ...
  function formatEth(value, digits = 4) {
    ...
    if (value >= 1000) return `${value.toLocaleString('en-US', { maximumFractionDigits: 2 })} ETH`;
  ...
  async function fetchJson(path) {
    const response = await fetch(`${CONFIG.noxaApiUrl}${path}`, { cache: 'no-store' });
  ...
  async function resolveToken() {
    ...
    const search = await fetchJson(
      `/v1/${CONFIG.chainSlug}/tokens/search?q=${encodeURIComponent(CONFIG.tokenName)}`
    );
    ...
    const symbolSearch = await fetchJson(
      `/v1/${CONFIG.chainSlug}/tokens/search?q=${encodeURIComponent(CONFIG.tokenSymbol)}`
    );
  ...
  function renderStats(token) {
    ...
    if (mcapDeltaEl && pct != null) {
      ...
      mcapDeltaEl.className = `text-xs mt-2 ${pct >= 0 ? 'text-green-400' : 'text-red-400'}`;
    }
    if (priceDeltaEl && pct != null) {
      const sign = pct >= 0 ? '▲' : '▼';
      priceDeltaEl.textContent = `${sign} ${Math.abs(pct).toFixed(1)}% 6h`;
      priceDeltaEl.className = `text-xs mt-2 ${pct >= 0 ? 'text-green-400' : 'text-red-400'}`;
    }
    ...
    if (graduationLabel) {
      graduationLabel.textContent = `${netBuy.toFixed(3)} / ${target} ETH net buy`;
    }

    const gradStatus = document.getElementById('graduation-status');
    if (gradStatus) {
      gradStatus.textContent = progress >= 100
  ...
  function renderPendingLaunch() {
    ...
    document.querySelectorAll('[data-stat="mcap"], [data-stat="price"], [data-stat="volume"]').forEach((el) => {
      el.textContent = '—';
    });
